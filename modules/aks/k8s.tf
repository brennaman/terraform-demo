...
resource "null_resource" "cluster4" {

    provisioner "local-exec" {
    command = <<EOT
    az login --service-principal --username ${var.AZURE_CLIENT_ID} --password ${var.AZURE_CLIENT_SECRET} --tenant ${var.AZURE_TENANT_ID}
    az aks get-credentials --resource-group ${azurerm_resource_group.k8s.name} --name ${azurerm_kubernetes_cluster.k8s.name} -f "~/.kube/config" --admin
    kubectl get nodes
    ls -a
    cat ~/.kube/config
    EOT
...
resource "kubernetes_cluster_role_binding" "kubernetes-dashboard-rule" {
  ...
  depends_on = [null_resource.cluster4]
